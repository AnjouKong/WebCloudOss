...
function renderRooms(floorRooms, roomW, roomH, roomBetwen, getStyleByData) {
  var tmp = {
    ...
    roomsDataEl: [],
    floorNumberGroup: new zrender.Group(),
  };
  tmp.roomsData.forEach(function (arr, i) {
    if( arr instanceof Array){
      tmp.roomsDataEl[i] = [];
      var yTmp = (tmp.roomsData.length - 1 - i) * (roomH + roomBetwen);
      tmp.floorNumberGroup.add(createFloorNumber((i+1) + '楼', 0, yTmp));
...
// 生成 楼层
function createFloorNumber(text, x, y) {
