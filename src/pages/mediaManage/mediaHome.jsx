...
import AddHome from './homeModal/addHome';
import EditHome from './homeModal/editHome';
...
class libraryManage extends Component {
  ...
  getInfoList = (currentPage, pageSize) => {
    Utils.request({
      url: `${window.PAY_API_HOST}/op/homepage/page`,
      method: 'post',
      data: {
  ...
  deleteInfo = (id) => {
    ...
    confirm({
      ...
      onOk: () => {
        Utils.request({
          url: `${window.PAY_API_HOST}/op/homepage/delete`,
          method: 'post',
          data: {
            homeIds: id
          }
    ...
    return (
      <Layout>
        <Breadcrumb style={{ margin: '12px 0' }}>
          ...
          <Breadcrumb.Item>媒资首页</Breadcrumb.Item>
        </Breadcrumb>
        ...
        <Content style={{ background: '#fff', padding: 24, margin: 0, minHeight: 280 }}>
          <div>
            ...
            />
            <AddHome onRef={ref => { this.addModal = ref; }} onOK={() => this.getInfoList(current, '10')} />
            <EditHome onRef={(ref) => { this.editModal = ref; }} onOK={() => this.getInfoList(current, '10')} />
          </div>
        </Content>
      </Layout>
    );
