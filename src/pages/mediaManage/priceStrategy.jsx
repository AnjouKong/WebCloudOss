...
import AddPrice from './priceModal/addPrice';
import EditPrice from './priceModal/editPrice';
...
class libraryManage extends Component {
  ...
  getInfoList = (currentPage, pageSize) => {
    Utils.request({
      url: `${window.PAY_API_HOST}/op/price/page`,
      method: 'post',
      data: {
    ...
    .then(res => {
      ...
      for (let i = 0; i < res.length; i += 1) {
        dataArray.push({
          ...
          name: `${res[i].name}`,
          formula: `${res[i].exp}`,
          defaultPrice: `${res[i].defaultprice}`,
  ...
  deleteInfo = (id) => {
    ...
    confirm({
      ...
      onOk: () => {
        Utils.request({
          url: `${window.PAY_API_HOST}/op/price/delete`,
          method: 'post',
          data: {
            selectIds: id
          }
  ...
  render() {
    const { selectedRowKeys, tableData, tableTotal, current, priceName, } = this.state;
    const rowSelection = {
      selectedRowKeys,
    ...
    return (
      <Layout>
        <Breadcrumb style={{ margin: '12px 0' }}>
          ...
          <Breadcrumb.Item>价格策略</Breadcrumb.Item>
        </Breadcrumb>
        <div style={{ height: 60, backgroundColor: '#fff', padding: '20px 20px 0px' }} >
          ...
          <div style={{ float: 'right' }}>
            <Input value={priceName} onChange={this.priceOnChange} placeholder="价格名称" style={{ width: 130, marginRight: 7 }} />
        ...
        <Content style={{ background: '#fff', padding: 24, margin: 0, minHeight: 280 }}>
          <div>
            ...
            />
            <AddPrice onRef={ref => { this.addModal = ref; }} onOK={() => this.getInfoList(current, '10')} />
            <EditPrice onRef={(ref) => { this.editModal = ref; }} onOK={() => this.getInfoList(current, '10')} />
          </div>
        </Content>
      </Layout>
    );
