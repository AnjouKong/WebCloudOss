...
class contentList extends Component {
  ...
  componentWillMount() {
    ...
    this.getContentList(this.props.type);
  }
  ...
  modalAddOk = () => {
    ...
    // 新建
    Utils.request({
      url: `${window.PAY_API_HOST}/op/ui/column/add`,
      method: 'post',
      data: {
    ...
    .then(res => {
      if (res && res.success) {
        ...
        this.getContentList(this.state.type);
      }
    })
    .catch(() => {
      message.error('新建失败');
  ...
  modalEditOk = () => {
    ...
    Utils.request({
      url: `${window.PAY_API_HOST}/op/ui/column/edit`,
      method: 'post',
      data: {
  ...
  del = () => {
    ...
    confirm({
      ...
      onOk: () => {
        Utils.request({
          url: `${window.PAY_API_HOST}/op/ui/column/delete`,
          method: 'post',
          data: {
            id: parseInt(this.formData.currentId, 10),
          }
      ...
      <div className="orgListPage">
        <div className="orgListBtn">
          <ul className="clear-fix">
            {
              privileges.indexOf(ROLE_PERMISSIONS_VALUE.contentManage.btn[0]) > -1 &&
              <li><Button type="primary" onClick={this.add}>新建</Button></li>
            }
            {
              privileges.indexOf(ROLE_PERMISSIONS_VALUE.contentManage.btn[1]) > -1 &&
              <li><Button type="primary" onClick={this.edit}>编辑</Button></li>
            }
            {
              privileges.indexOf(ROLE_PERMISSIONS_VALUE.contentManage.btn[2]) > -1 &&
