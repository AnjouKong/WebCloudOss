...
class addPrice extends Component {
  ...
  modalOk = () => {
    ...
    Utils.request({
      url: `${window.PAY_API_HOST}/op/node/add`,
      method: 'post',
      data: {
  ...
  render() {
    const { modalVisible, selectChildren, nodeName, tenantId, memory, disk, bandWidth, cachePath, cacheSize, mediaScheduledClose,
      monitorSwitch, monitorScheduled, downloadSwitch, mediaScheduled, rateLimit, maxTaskSize, downloadSpeed, tsSize, proxyUrl, mediaTime } = this.state;
    return (
      <Modal title="新建节点" visible={modalVisible} onOk={this.modalOk} onCancel={this.modalCancel} width="1080px" >
        <Row>
          <Col span={12} className="nodeInfo">
            <span className="required">名称：</span>
            <Input value={nodeName} onChange={this.nameOnChange} placeholder="名称" />
          </Col>
          <Col span={12} className="nodeInfo">
            <span>商户：</span>
          ...
          <Col span={12} className="nodeInfo">
            ...
            <Input value={memory} onChange={this.memoryOnChange} placeholder="内存大小/G" />
          </Col>
          <Col span={12} className="nodeInfo">
            <span>硬盘大小：</span>
            <Input value={disk} onChange={this.diskOnChange} placeholder="硬盘大小/G" />
          </Col>
          <Col span={12} className="nodeInfo">
            <span className="required">带宽大小：</span>
            <Input value={bandWidth} onChange={this.bandWidthOnChange} placeholder="带宽大小/M" />
          </Col>
          <Col span={12} className="nodeInfo">
            <span className="required">缓存区地址：</span>
            <Input value={cachePath} onChange={this.cachePathOnChange} placeholder="缓存区地址" />
          </Col>
          <Col span={12} className="nodeInfo">
            <span>缓存区大小：</span>
            <Input value={cacheSize} onChange={this.cacheSizeOnChange} placeholder="缓存区大小/G" />
          </Col>
          <Col span={12} className="nodeInfo">
